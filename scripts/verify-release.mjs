...
for (const required of ['LICENSE', 'THIRD_PARTY_NOTICES.md', 'third_party/SOURCES.md', 'third_party/sources.lock.json', 'third_party/licenses/ffmpeg/COPYING.GPLv2', 'third_party/licenses/react/LICENSE']) assert.ok(extension[required]?.length, `Missing ${required}`);
