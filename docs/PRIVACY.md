...
适用版本 / Version: **0.1.11** · 更新日期 / Updated: **2026-09-20**

## 数据处理

Gif Toolkit 不设账号、统计上报、广告追踪或转码服务器。编码器随扩展本地打包，网页画面和生成的 GIF 在浏览器所在设备处理。

- **页面读取**：读取用户当前操作或明确授权网站中的媒体元素、标题、来源链接、尺寸及时间信息。所有视频先下载，在扩展工作线程中解码；不再逐帧拖动页面播放器。
...
- **删除**：历史页“清理缓存”只删除扩展内的 GIF 文件，保留任务元数据；“清空历史”或删除任务会删除对应元数据和文件。自动保留最近 30 个结束任务；原始输入和处理临时数据在任务结束后释放。卸载扩展会清除其本地数据。已保存到下载目录的文件需自行删除；扩展不清除 Chrome 的全局缓存或其他网站数据。
